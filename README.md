...
- **Deep format support:**
  ...
  - **PDF** — removes the Document Information dictionary (Title, Author, Subject, Keywords, Creator, Producer, dates, and any custom fields) and the embedded XMP metadata packet, while leaving page content untouched. PDFs also get a **page view**: see every page as a thumbnail, mark the ones you don't want, and download a copy with just those pages removed.
...
- **PDF** documents are parsed with [pdf-lib](https://pdf-lib.js.org/) — the Info dictionary and the catalog's XMP metadata reference are removed, and the file is rewritten so orphaned metadata objects aren't just unlinked but actually excluded from the output. Page thumbnails for the page-removal view are rendered with [pdf.js](https://mozilla.github.io/pdf.js/).
